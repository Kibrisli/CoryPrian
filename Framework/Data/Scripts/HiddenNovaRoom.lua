...
local function OnBeginOverlap(trigger, player)
	...
	if isOutside then
		...
		local hasKey = _G.QuestController.HasCompleted(player, "Lv60")
		if not hasKey then
			Events.BroadcastToPlayer(player, "HiddenNova.LockedMessage")
			return
		end
		
		Events.Broadcast("Quest.HiddenNova", player)
	end
