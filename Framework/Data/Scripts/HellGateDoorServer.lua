...
local function OnBeginOverlap(trigger, player)
	...
	if isOutside then
		...
		local hasKey = _G.QuestController.HasCompleted(player, "Lv10")
		if not hasKey then
			Events.BroadcastToPlayer(player, "HellGate.LockedMessage")
			return
		end
		
		Events.Broadcast("Quest.HellGateEnter", player)
	end
